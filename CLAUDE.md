...
## SPÉCIFICITÉS DU PROJET (§15.4) — dépôt, stack, vérifications, pièges

**Dépôt** : `chaima0007/keywordmoneymaker` — branche principale `main`.
Un push sur `main` **déploie le site en ligne** (GitHub Actions → GitHub Pages, domaine
`caelumpartners.agency`). Donc : jamais de commit direct sur `main` (§8 Parcours 4, §10).

**Stack** : site **statique**, sans build ni framework — HTML + CSS + JS natif.
Feuille de style et scripts partagés dans `/assets/` (`caelum.css`, `caelum.js`, `simulateur.js`).
Polices : Google Fonts (Fraunces + Inter). Aucun cookie, aucun traceur, aucune donnée envoyée.

**Commandes de vérification AVANT push** (il n'y a ni lint ni test unitaire sur ce dépôt statique) :

    # 1. Les HTML parsent sans balise orpheline
    python3 -c "from html.parser import HTMLParser; [HTMLParser().feed(open(f,encoding='utf-8').read()) for f in ['caelum-index.html','simulateur.html','offres.html','mentions-legales.html']]"
    # 2. Aucune formulation bannie (positionnement TRANCHÉ le 2026-07-17)
    grep -riE "9 agents|d'élite|agence digitale" *.html && echo "REJET" || echo "OK"
    # 3. CSP présente sur chaque page + aucun script inline
    grep -L "Content-Security-Policy" *.html ; grep -c "<script>" *.html
    # 4. Aucun secret
    grep -riE "sk_live|api[_-]?key|password" --include=*.html --include=*.js .

**Pièges connus (constatés, datés)** :
- **2026-08-10** — La CSP `<meta>` de chaque page vaut `script-src 'self'` : **tout script inline
  est bloqué**. Le JS doit vivre dans `/assets/*.js`. Les polices exigent en plus
  `fonts.googleapis.com` (style-src) et `fonts.gstatic.com` (font-src).
- **2026-08-10** — Les liens sont **absolus depuis la racine** (`/simulateur.html`, `/assets/…`) :
  le site suppose un domaine racine. Sous une URL de type `/nom-du-depot/`, la navigation casserait.
- **2026-08-10** — Le rendu visuel réel (polices Google, responsive) **n'est pas vérifiable depuis
  le sandbox** (sortie réseau restreinte). Toute affirmation « le design s'affiche bien » est
  **NON VÉRIFIÉ** tant qu'un humain n'a pas regardé.
- **2026-09-06** — Vérifier la branche courante avant de qualifier un fichier d'anormal : les
  branches anciennes portent encore l'ancien site (cf. 🔴 ERREURS.md).
